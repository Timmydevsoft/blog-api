...
const signIn = async (req, res, next) => {
  try {
    ...
    const accessToken = jwt.sign({id:user._id, role:user.role}, process.env.ACCESS_KEY, {expiresIn: "1h"},)
    ...
    ).status(200).json({success: true, token: accessToken, userName: user.userName})
...
const refreshToken = async(req, res, next)=>{
  try{
    ...
    return res.status(200).json({sucessful: true, accessToken: newAccessToken})
  }
  catch(err){
    next(err)
  }
